...
import { GET_PRODUCTS, GET_CATEGORIES } from "@/gql/Product";
...
const ShopPage: React.FC = () => {
  ...
  const {
    ...
    variables: { first: pageSize, skip: 0, filters: initialFilters },
  ...
  // Sync displayed products when query data changes
  useEffect(() => {
    if (productsData?.products) {
      setDisplayedProducts(productsData.products.products);
      setHasMore(productsData.products.hasMore);
      setSkip(0);
    }
  }, [productsData]);

  // Update filters and trigger refetch when initialFilters (from URL) change
  useEffect(() => {
    setFilters(initialFilters);
    // Note: refetching here is redundant if variables are correctly synced with initialFilters,
    // but we do it to ensure fresh data on URL change.
    refetch({ first: pageSize, skip: 0, filters: initialFilters });
  }, [initialFilters, refetch]);
